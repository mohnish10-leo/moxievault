...
export default function PublicVaultByNamePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (error) {
        console.error("PUBLIC VAULT FETCH ERROR:", error);
        setMessage("Vault not found or not public.");
        setLoading(false);
        return;
      }

      if (!data.is_public) {
        setMessage("This vault is private. Use the access token.");
        setLoading(false);
        return;
      }
      ...
      if (filesError) {
        console.error("PUBLIC FILES FETCH ERROR:", filesError);
      } else {
        setFiles(filesData ?? []);
      }
      ...
      <div className="mx-auto max-w-4xl">
        <div className="vault-card vault-shadow rounded-2xl p-6 sm:p-8 fade-up">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <Logo />
            <a href="/discover" className="text-xs text-white/70 hover:text-white">
              Back to discovery
            </a>
          </div>
